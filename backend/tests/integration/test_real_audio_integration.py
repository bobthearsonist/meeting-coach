...
class TestRealAudioIntegration:
    ...
    def test_real_audio_transcription(self, test_audio_file):
        ...
        if not os.path.exists(test_audio_file):
            pytest.skip(
                "test_capture.wav not found - run 'python main.py --test-audio' first"
            )
    ...
    def test_real_audio_analysis_pipeline(self, test_audio_file):
        ...
        if transcription_result["word_count"] >= config.MIN_WORDS_FOR_ANALYSIS:
            ...
            assert analysis_result["emotional_state"] in [
                "supportive",
                "dismissive",
                "neutral",
                "aggressive",
            ...
            print(
                f"Transcription too short ({transcription_result['word_count']} words) for analysis"
            )
...
class TestFullEndToEndPipeline:
    ...
    def test_synthetic_audio_pipeline(self):
        ...
        if transcription["duration"] > 0 and transcription["word_count"] > 0:
            wpm = transcriber.calculate_wpm(
                transcription["word_count"], transcription["duration"]
            )
        ...
        filler_counts = transcriber.count_filler_words(
            "um like actually this is a test"
        )
        ...
        assert (
            "um" in filler_counts
            or "like" in filler_counts
            or "actually" in filler_counts
        )
    ...
    def test_filler_word_detection_with_punctuation(self):
        ...
        filler_counts = transcriber.count_filler_words(
            "Um, like, you know, this is a test"
        )
        ...
        assert (
            complex_counts.get("you know", 0) >= 1
        ), "Should detect 'you know?' as 'you know'"
        assert (
            complex_counts.get("actually", 0) >= 1
        ), "Should detect 'Actually,' as 'actually'"
        assert (
            complex_counts.get("basically", 0) >= 1
        ), "Should detect 'basically!' as 'basically'"
