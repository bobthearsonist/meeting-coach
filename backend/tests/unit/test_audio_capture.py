...
class TestAudioCapture:
    ...
    def test_read_chunk_multiple_reads(self, mock_pyaudio_class, mock_pyaudio, sample_audio_bytes):
        ...
        result = capture.read_chunk(0.5)  # Use shorter duration to avoid too many mock calls
    ...
    @pytest.mark.parametrize("channels,sample_rate", [
        (1, 16000),
        (2, 44100),
        (2, 48000),
    ])
    ...
    def test_audio_data_conversion(self, mock_pyaudio_class, mock_pyaudio):
        ...
        if config.CHANNELS == 2:
            int16_data = np.array([32767, -32768, 0, 16384, -16384, 8192], dtype=np.int16)
        else:
        ...
        np.testing.assert_allclose(result[:min_len], expected_values[:min_len], rtol=1e-5)
